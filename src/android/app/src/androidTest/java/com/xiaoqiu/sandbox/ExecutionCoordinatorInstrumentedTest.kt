...
class ExecutionCoordinatorInstrumentedTest {
    ...
    fun executeCreatesSessionBindMounts() = runBlocking {
        ...
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/attachments"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/offloads"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/workspace"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/browser"))

        // Verify global mounts
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/memory"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/xiaoqiu/skills"))
    }
    ...
    fun sessionBindMountsPointToCorrectHostDirs() = runBlocking {
        ...
        assertEquals(
            ...
            PRootKernel.bindMounts["/var/xiaoqiu/workspace"]
        )

        val globalBase = File(context.filesDir, "xiaoqiu-global")
        assertEquals(
            File(globalBase, "memory").absolutePath,
            PRootKernel.bindMounts["/var/xiaoqiu/memory"]
        )
    }
    ...
    fun differentSessionsHaveIsolatedWorkspaces() = runBlocking {
        ...
        ExecutionCoordinator.execute(
            "session-iso-A",
            "echo 'from A' > /var/xiaoqiu/workspace/test.txt"
        )

        // Check it exists in session A
        val resultA = ExecutionCoordinator.execute(
            "session-iso-A",
            "cat /var/xiaoqiu/workspace/test.txt"
        )
    ...
    private fun cleanupSessionDirs() {
        listOf("session-mount-test", "session-A", "session-B", "session-X",
            ...
            File(context.filesDir, "xiaoqiu-sessions/$id").deleteRecursively()
        }
        File(context.filesDir, "xiaoqiu-global").deleteRecursively()
    }
}
