package com.xiaoqiu.config.collections

import com.xiaoqiu.config.ConfigCollection
import com.xiaoqiu.config.ConfigError
import com.xiaoqiu.config.ConfigField
import com.xiaoqiu.config.ConfigRisk
import com.xiaoqiu.config.ConfigSchema
import com.xiaoqiu.config.ConfigValue
import com.xiaoqiu.config.fields.ClosureField
import com.xiaoqiu.config.fields.HiddenField
 ...
 *
 * [T-xiaoqiu-config-provider-add] Add is OPEN (per user decision: writes
 ...
 *     from a multi-step browser flow that xiaoqiu-config cannot drive.
 */
class ProvidersCollection(
    ...
    override fun fields(forId: String): List<ConfigField> {
        ...
        return listOf(
            ...
            HiddenField(
                ...
                reason = "OAuth tokens cannot be set via xiaoqiu-config — use Settings UI",
            ),
        )
    }
        ...
        val resolved = envVars.getValue(key)
            ?: throw ConfigError.InvalidValue(
                "$fieldName references env var \$\$$key, but no such env var exists. " +
                    "Create it first via [Set $key](xiaoqiu://settings/environments?create_key=$key&create_value=)."
            )
        if (resolved.isEmpty()) {
    ...
    /**
     * [T-android-xiaoqiuconfig-custom-useragent] Per-provider User-Agent
     * override. Empty string clears the override and falls back to the
     * branded default (XiaoQiu/<version> (Android <release>; <model>) — see
     * XiaoQiuUserAgent.DEFAULT) at request build time. Cloned from
     ...
     * `read_failed`, thanks to the explicit catch added in T-xiaoqiu-
    ...
    private fun apiKeyField(id: String): ConfigField =
        ClosureField(
            ...
            reader = {
                throw ConfigError.PermissionDenied(
                    "API keys are never read back via xiaoqiu-config. To change one, write a new value."
                )
            },
            writer = { v ->
                val s = (v as? ConfigValue.Str)?.value ?: throw ConfigError.TypeMismatch("string")
