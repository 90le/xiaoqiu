...
/**
 * Coil Fetcher that resolves `xiaoqiu://` URIs to local files.
 *
 * Usage: Register with ImageLoader.Builder().components {
 *     add(XiaoQiuImageFetcher.Factory())
 * }
 *
 * xiaoqiu://attachments/foo.jpg → /var/xiaoqiu/attachments/foo.jpg → host path
 */
class XiaoQiuImageFetcher(
    private val uri: String,
    private val options: Options,
) : Fetcher {

    override suspend fun fetch(): FetchResult {
        // Strip xiaoqiu:// prefix, drop any ?query, and percent-decode so
        // Chinese/emoji/space filenames resolve to the actual on-disk file.
        val stripped = uri.removePrefix("xiaoqiu://").substringBefore('?')
    ...
    class Factory : Fetcher.Factory<String> {
        override fun create(data: String, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (!data.startsWith("xiaoqiu://")) return null
            return XiaoQiuImageFetcher(data, options)
        }
    }

    /**
     * Uri factory — the path the Markdown renderer hits. Coil's default
     * StringMapper converts an `AsyncImage(model = "xiaoqiu://…")` String into
    ...
    class UriFactory : Fetcher.Factory<Uri> {
        override fun create(data: Uri, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (data.scheme != "xiaoqiu") return null
            return XiaoQiuImageFetcher(data.toString(), options)
        }
    }

    /**
     * T-image-cache-mtime-35133: Bust Coil's memory + disk cache when a
     * `xiaoqiu://` file is overwritten in-place (e.g. Grok regenerating an
     ...
     *
     * The key composes `<xiaoqiu-uri>?mt=<lastModified>`. The fetcher above
     ...
     * to Coil's default keying, which is correct for non-xiaoqiu data.
     *
