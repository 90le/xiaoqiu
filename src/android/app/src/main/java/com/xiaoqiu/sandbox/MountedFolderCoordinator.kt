...
object MountedFolderCoordinator {
    ...
    private const val MOUNTS_PREFIX = "/var/xiaoqiu/mounts/"

    /**
     * Throws [ReadOnlyMountException] when [linuxPath] is inside an
     * effectively read-only mount. Paths outside `/var/xiaoqiu/mounts/` and
    ...
    /**
     * True when [linuxPath] is `/var/xiaoqiu/mounts/<name>/...` AND the
    ...
    fun isLinuxPathUnderReadOnlyMount(linuxPath: String, store: MountedFoldersStore): Boolean {
        ...
        // `/var/xiaoqiu/mounts/foobar` as inside a mount named `foo`.
