 ...
 *     also accepts a Linux file path under /var/xiaoqiu/... resolved via
...
class SpeechOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun cmdTranscribe(args: OffloadArgs): NativeOffloadResult {
        ...
        // --source: default mic, accept "mic" / "system-mic" / any /var/xiaoqiu/... path.
    ...
    /**
     * Resolve a `/var/xiaoqiu/...`-style Linux path to a host File via
            ...
            if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                r = OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "XiaoQiu needs microphone permission to transcribe speech. Open Settings to allow it.",
...
EXAMPLES:
  ...
  android-speech transcribe --source /var/xiaoqiu/attachments/meeting.m4a
  android-speech languages
