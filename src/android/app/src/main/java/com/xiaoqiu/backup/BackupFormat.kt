...
/**
 * On-the-wire types for the `.xiaoqiubak` backup package.
 *
...
object BackupFormat {
    ...
    const val CURRENT = "xiaoqiubak/1"

    /** File extension registered to the app for "open to import". */
    const val FILE_EXTENSION = "xiaoqiubak"

    /**
     * MIME type for a `.xiaoqiubak` package.
     *
     * Matches the `application/x-xiaoqiubak` iOS declares for the
     * `com.xiaoqiu.xiaoqiubak` UTI (src/ios/Info.plist), so both platforms
     ...
     */
    const val MIME_TYPE = "application/x-xiaoqiubak"
