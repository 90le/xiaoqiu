package com.xiaoqiu.provider.openai
...
import com.xiaoqiu.data.model.LLMMediaAttachment
import com.xiaoqiu.data.model.LLMMessage
import com.xiaoqiu.data.model.LLMModel
import com.xiaoqiu.data.model.LLMResponse
...
import com.xiaoqiu.provider.applyUserAgentOverride
...
import com.xiaoqiu.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.xiaoqiu.provider.thinking.ThinkingRuleResolver]'s
     ...
     * the app-level [com.xiaoqiu.data.FastModePrefs] toggle that Codex
    ...
    internal fun resolvedServiceTier(): String? =
        if (supportsPriorityProcessing && com.xiaoqiu.data.FastModePrefs.isEnabled()) {
            "priority"
        } else {
            null
        }
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.xiaoqiu.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.xiaoqiu.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.xiaoqiu.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.xiaoqiu.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.xiaoqiu.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.xiaoqiu.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.xiaoqiu.BuildConfig.DEBUG) {
                com.xiaoqiu.debug.LLMRequestLog.add(
                    com.xiaoqiu.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.xiaoqiu.logging.AppLogger.debug(
                                "OpenAIProvider",
                    ...
                    msg.role == LLMMessage.Role.USER -> {
                        ...
                        for (tr in toolResults) {
                            ...
                            if (trBytes != null && trBytes.isNotEmpty() && supportsImages) {
                                val safeBytes = com.xiaoqiu.provider.ImageBudget
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.xiaoqiu.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.xiaoqiu.provider.ImageBudget.compressUnderBudget(data)
    ...
    internal fun buildResponsesAPIBody(
        ...
        if (!body.has("service_tier") &&
            com.xiaoqiu.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.xiaoqiu.logging.AppLogger.error(
                                        "OpenAIProvider",
