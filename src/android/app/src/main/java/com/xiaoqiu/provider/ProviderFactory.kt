...
import com.xiaoqiu.provider.anthropic.AnthropicProvider
import com.xiaoqiu.provider.gemini.GeminiProvider
import com.xiaoqiu.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI, ProviderType.openAIResponses -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.xiaoqiu.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.xiaoqiu.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.openRouter -> {
                ...
                OpenAIProvider(
                    ...
                    extraHeaders = mapOf(
                        "HTTP-Referer" to "https://github.com/OpenXiaoQiu/OpenXiaoQiu",
                        "X-Title" to "XiaoQiu App",
                    ),
                )
            }
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.xiaoqiu.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.xiaoqiu.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.xiaoqiu.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.xiaoqiu.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.xiaoqiu.auth.KimiOAuthManager(context, instance.id)
