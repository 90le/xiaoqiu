package com.xiaoqiu.webapp

import android.content.Context
import com.xiaoqiu.data.db.WebAppShortcutEntity
 ...
 *      under /var/xiaoqiu/<sub>/ are also accepted via resolveSessionHostPath)
...
object WebAppPathResolver {
     ...
     *
     * Mapping rules:
     *  - `/var/xiaoqiu/shared` bind  → `pathScope = "shared"`,  `scopeContext = null`
     *  - `/var/xiaoqiu/mounts/<n>`   → `pathScope = "mount"`,   `scopeContext = "<n>"`
    ...
    fun inferScope(hostFile: File): Triple<String, String?, String>? {
        ...
        val sorted = com.xiaoqiu.sandbox.PRootKernel
        ...
        for ((linuxPrefix, hostBase) in sorted) {
            ...
            if (hostAbs == baseNorm || hostAbs.startsWith("$baseNorm/")) {
                ...
                return when {
                    linuxPrefix == "/var/xiaoqiu/shared" ->
                        Triple(WebAppShortcutRepository.SCOPE_SHARED, null, linuxPath)
                    linuxPrefix.startsWith("/var/xiaoqiu/mounts/") -> {
                        val mountName = linuxPrefix.removePrefix("/var/xiaoqiu/mounts/")
                            .substringBefore('/')
    ...
    private fun resolveSession(context: Context, shortcut: WebAppShortcutEntity): File? {
        ...
        // Absolute /var/xiaoqiu/<perSession>/... — go through PRootKernel which
        // knows how to map per-session subdirs to <filesDir>/xiaoqiu-sessions/<id>/<sub>.
        if (shortcut.htmlPath.startsWith("/var/xiaoqiu/")) {
