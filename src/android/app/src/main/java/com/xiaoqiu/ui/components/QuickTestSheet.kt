...
import com.xiaoqiu.data.model.normalizeModalityName
...
internal suspend fun performTest(
    ...
    if (kind == QuickTestKind.SPEECH_OUT || kind == QuickTestKind.TRANSCRIPTION) {
        val voice = com.xiaoqiu.provider.voice.VoiceProviderFactory.make(instance, apiKey)
                ...
                if (!voice.supportsVoiceInput) {
                    ...
                    runCatching {
                        val resp = voice.transcribe(
                            com.xiaoqiu.provider.voice.VoiceInputRequest(
                                audioData = clip,
