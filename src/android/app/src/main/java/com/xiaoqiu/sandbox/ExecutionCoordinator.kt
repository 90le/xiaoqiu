...
object ExecutionCoordinator {
    ...
    private suspend fun getOrCreateShell(sessionId: String): PersistentShell {
        ...
        if (existing != null && existing.isAlive) {
            Log.w(TAG, "[diag] reuse existing shell for sessionId=$sessionId attachmentsMount=${existing.debugBindMount("/var/xiaoqiu/attachments")}")
            return existing
        }
        ...
        return globalLock.withLock {
            ...
            if (recheck != null && recheck.isAlive) {
                Log.w(TAG, "[diag] reuse existing shell (post-lock) for sessionId=$sessionId attachmentsMount=${recheck.debugBindMount("/var/xiaoqiu/attachments")}")
            ...
            Log.w(TAG, "[diag] new shell created sessionId=$sessionId attachmentsMount=${bindMounts["/var/xiaoqiu/attachments"]}")
            shell
        }
    }
    ...
    private fun buildSessionBindMounts(sessionId: String): Map<String, String> {
        ...
        val prevAttachments = PRootKernel.bindMounts["/var/xiaoqiu/attachments"]

        // Session-specific directories
        val sessionBase = File(filesDir, "xiaoqiu-sessions/$sessionId")
        listOf("attachments", "offloads", "workspace", "browser").forEach { subdir ->
            val hostDir = File(sessionBase, subdir).also { it.mkdirs() }
            val linuxPath = "/var/xiaoqiu/$subdir"
        ...
        Log.w(TAG, "[diag] buildSessionBindMounts sessionId=$sessionId " +
            "attachments: prev=$prevAttachments new=${mounts["/var/xiaoqiu/attachments"]}")
        ...
        // shell that runs xiaoqiu-mcp-cli — /var/xiaoqiu/mcp-servers/servers.json
        // then resolves to the empty rootfs placeholder and `xiaoqiu-mcp-cli list`
        ...
        // below.
        val globalBase = File(filesDir, "xiaoqiu-global")
        ...
        // is invisible to the shell — `ls /var/xiaoqiu/mounts/<name>/` then shows
        ...
        PRootKernel.mountedFoldersStore?.entries?.value?.forEach { entry ->
            ...
            val linuxPath = "/var/xiaoqiu/mounts/${entry.name}"
