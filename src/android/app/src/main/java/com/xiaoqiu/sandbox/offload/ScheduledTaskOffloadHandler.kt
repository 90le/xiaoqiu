...
import com.xiaoqiu.scheduled.ScheduledTaskManager
...
/**
 * [T-android-scheduled-tasks-full] xiaoqiu-scheduled — CLI surface for the
 ...
 *
 *   xiaoqiu-scheduled list
 *   xiaoqiu-scheduled create --label L --time HH:MM --prompt "..."
 ...
 *                          [--disabled]
 *   xiaoqiu-scheduled delete --id <taskId>
 *   xiaoqiu-scheduled enable  --id <taskId>
 *   xiaoqiu-scheduled disable --id <taskId>
 *   xiaoqiu-scheduled run     --id <taskId>     (fire immediately, off-schedule)
 *
...
class ScheduledTaskOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional.firstOrNull() ?: "list") {
                ...
                "run" -> handleRun(args)
                else -> NativeOffloadResult(2, "xiaoqiu-scheduled: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: IllegalArgumentException) {
            NativeOffloadResult(2, "xiaoqiu-scheduled: ${e.message}")
        } catch (e: Throwable) {
            AppLogger.warning(TAG, "handle failed: ${e.message}")
            NativeOffloadResult(1, "xiaoqiu-scheduled: ${e.message}")
        }
    }
    ...
    private fun handleDelete(args: OffloadArgs): NativeOffloadResult {
        ...
        if (manager.get(id) == null) return NativeOffloadResult(1, "xiaoqiu-scheduled: no task with id=$id")
        manager.delete(id)
    ...
    private fun handleRun(args: OffloadArgs): NativeOffloadResult {
        val id = args.get("id") ?: throw IllegalArgumentException("--id required")
        val task = manager.get(id) ?: return NativeOffloadResult(1, "xiaoqiu-scheduled: no task with id=$id")
        ...
        val sessionId = runBlocking {
            com.xiaoqiu.scheduled.ScheduledAgentRunner.run(context, task)
        }
        val out = JSONObject()
            .put("id", id)
    ...
    companion object {
        ...
        private val HELP = """
            xiaoqiu-scheduled — manage timed AI tasks (mirrors the in-app Scheduled Tasks).
