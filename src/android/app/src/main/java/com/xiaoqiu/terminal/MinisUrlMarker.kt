package com.xiaoqiu.terminal

/**
 * Parses the OSC 1337 `XiaoQiuOpenURL` escape sequence emitted by the rootfs
 * shim at `/usr/local/bin/xiaoqiu-open`. The shim stands in for `xdg-open`,
 * `sensible-browser`, `www-browser`, and `$BROWSER`; whenever an in-sandbox
 * command tries to open a URL, it prints:
 *
 *     ESC ] 1337 ; XiaoQiuOpenURL = <url> BEL
 *
 ...
 * http(s)/about, file preview for xiaoqiu://).
 *
 * Mirrors iOS `XiaoQiuURLMarker` in `AIChatViewModel.swift`.
 */
object XiaoQiuUrlMarker {
    /**
     * ESC ] 1337 ; XiaoQiuOpenURL = <url> (BEL | ESC \)
     *
     ...
     */
    private val PATTERN = Regex("\u001B]1337;XiaoQiuOpenURL=([^\u0007\u001B]*)(?:\u0007|\u001B\\\\)")
    ...
    fun extract(text: String): Pair<String, List<String>> {
        if (!text.contains("XiaoQiuOpenURL=")) return text to emptyList()
    ...
    fun stripMarkers(text: String): String {
        if (!text.contains("XiaoQiuOpenURL=")) return text
    ...
    fun extractUrl(text: String): String? {
        if (!text.contains("XiaoQiuOpenURL=")) return null
