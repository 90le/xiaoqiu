...
object SoulIcon {
    ...
    /**
     * [T-android-soul-icon-config-images] Turn a `xiaoqiu-config` value into
     * bitmap bytes.
     *
    ...
    fun classifySource(raw: String): Source {
        ...
        return when {
            v.startsWith("http://", true) || v.startsWith("https://", true) ->
                Source.Unsupported(
                    "remote URLs aren't supported on Android — download the file first, " +
                        "then pass a path like /var/xiaoqiu/attachments/icon.png",
                )
            v.startsWith("xiaoqiu://") -> {
                // xiaoqiu://attachments/x.png -> /var/xiaoqiu/attachments/x.png
                val rest = v.removePrefix("xiaoqiu://").trimStart('/')
                if (rest.isEmpty()) Source.Unsupported("empty xiaoqiu:// path")
                else Source.LinuxPath("/var/xiaoqiu/$rest")
            }
            v.startsWith("data:") -> {
                val comma = v.indexOf(',')
            ...
            looksLikeBareBase64(v) ->
                ...
                "not an emoji, a data URI, base64, a xiaoqiu:// resource or a /var/xiaoqiu path",
            )
        }
    }
     ...
     * model-supplied `/var/xiaoqiu/attachments/../../../databases/x` would walk
     * straight out of the sandbox.
     */
    val ALLOWED_LINUX_ROOTS = listOf(
        "/var/xiaoqiu/attachments",
        "/var/xiaoqiu/workspace",
        "/var/xiaoqiu/offloads",
        "/var/xiaoqiu/shared",
        "/var/xiaoqiu/memory",
        "/var/xiaoqiu/skills",
    )
