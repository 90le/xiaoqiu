...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.xiaoqiu.data.repository.EnvVarRepository? = null,
) {
    val servers by mcpRepository.servers.collectAsState()

    // [T-android-mcp-list-reload-on-appear] MCPRepository reads servers.json
    // only in init() (app launch). A server the agent writes via xiaoqiu-mcp-cli
    ...
    if (deleteId != null) {
        ...
        AlertDialog(
            ...
            dismissButton = {
                XiaoQiuTextButton(onClick = { deleteId = null }) { Text(stringResource(R.string.cancel)) }
            },
        )
    }
}
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.xiaoqiu.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.xiaoqiu.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.xiaoqiu.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.xiaoqiu.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.xiaoqiu.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.xiaoqiu.mcp.oauth.MCPOAuthConfig(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (oauthAuthorized) {
                        XiaoQiuTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.xiaoqiu.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.xiaoqiu.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.xiaoqiu.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.xiaoqiu.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.xiaoqiu.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.xiaoqiu.mcp.oauth.MCPOAuthController.Result.Failed -> {
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            XiaoQiuTextButton(onClick = {
                val server = buildServerFromForm() ?: return@XiaoQiuTextButton
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            if (isEdit) {
                XiaoQiuTextButton(onClick = onRequestDelete) {
                    Text(stringResource(R.string.delete), color = MaterialTheme.colorScheme.error)
                }
            }
            Spacer(Modifier.weight(1f))
            XiaoQiuTextButton(onClick = onDone) { Text(stringResource(R.string.cancel)) }
            XiaoQiuTextButton(onClick = {
                val server = buildServerFromForm() ?: return@XiaoQiuTextButton
    ...
    if (showClearEnvConfirm) {
        AlertDialog(
            ...
            dismissButton = {
                XiaoQiuTextButton(onClick = { showClearEnvConfirm = false }) {
