...
internal object ProviderDebugMethods {
            ...
            if (includeMembers) {
                ...
                for (memberId in group.memberEntryIds) {
                    ...
                    val entry = cfg.modelEntries.find { it.id == memberId }
                        ?: com.xiaoqiu.data.model.SystemVoiceEntries.resolve(memberId)
                        ?: continue
    ...
    suspend fun quickTest(context: Context, params: JSONObject): JSONObject {
        ...
        val entry = repo.config.value.modelEntries.find { it.id == entryId }
            ?: com.xiaoqiu.data.model.SystemVoiceEntries.resolve(entryId)
            ?: throw RPCException(-32602, "Model entry not found: $entryId")

        val requested = params.optJSONArray("kinds")
        val kinds: List<com.xiaoqiu.ui.components.QuickTestKind> = if (requested == null) {
            com.xiaoqiu.ui.components.applicableKinds(entry)
        } else {
            (0 until requested.length()).mapNotNull { i ->
                when (requested.optString(i).lowercase()) {
                    "text" -> com.xiaoqiu.ui.components.QuickTestKind.TEXT
                    "speechout", "speech_out", "tts" -> com.xiaoqiu.ui.components.QuickTestKind.SPEECH_OUT
                    "transcription", "asr" -> com.xiaoqiu.ui.components.QuickTestKind.TRANSCRIPTION
                    "imagegen", "image_gen", "image" -> com.xiaoqiu.ui.components.QuickTestKind.IMAGE_GEN
                    else -> null
                }
            }
        }
        ...
        for (kind in kinds) {
            ...
            val state = try {
                com.xiaoqiu.ui.components.performTest(kind, entry, repo, context)
            } catch (e: Exception) {
                com.xiaoqiu.ui.components.QuickTestState.Failure(
            ...
            results.put(JSONObject().apply {
                ...
                when (state) {
                    is com.xiaoqiu.ui.components.QuickTestState.TextReply -> {
                        put("status", "ok"); put("detail", state.text.take(400))
                    }
                    is com.xiaoqiu.ui.components.QuickTestState.AudioReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "audio ${state.data.size} bytes")
                    }
                    is com.xiaoqiu.ui.components.QuickTestState.ImageReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "image ${state.data.size} bytes")
                    }
                    is com.xiaoqiu.ui.components.QuickTestState.Failure -> {
