...
/**
 * Passphrase-based encryption for `.xiaoqiubak` packages
 * (docs/backup-restore-design.md §5, scheme `xiaoqiubak-enc/1`).
 *
 ...
 *     block) with infos `xiaoqiubak/{data,secrets,mac,verify}`.
...
object BackupCrypto {
    ...
    const val SCHEME = "xiaoqiubak-enc/1"
    ...
    class Keys internal constructor(kek: ByteArray) {
        ...
        val dataKey: ByteArray = hkdfSha256(kek, "xiaoqiubak/data")
        /**
         ...
         */
        val secretsKey: ByteArray = hkdfSha256(kek, "xiaoqiubak/secrets")
        /** Authenticates manifest.json. */
        val macKey: ByteArray = hkdfSha256(kek, "xiaoqiubak/mac")
        /** Answers "is this passphrase right?" without touching any payload. */
        val verifierKey: ByteArray = hkdfSha256(kek, "xiaoqiubak/verify")
        ...
        /**
         * `HMAC(verifier_key, "xiaoqiubak-v1")` truncated to 16 bytes, base64
        ...
        val verifier: String
            get() {
                val mac = hmacSha256(verifierKey, "xiaoqiubak-v1".toByteArray(Charsets.UTF_8))
