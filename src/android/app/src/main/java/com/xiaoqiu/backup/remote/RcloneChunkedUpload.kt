package com.xiaoqiu.backup.remote
 ...
 * `.xiaoqiu-parts/<backupId>/` with a resume journal, because rclone has no
 ...
 * and invisible to the iOS restore picker, which lists `.xiaoqiubak` files only.
 ...
 * self-contained `.xiaoqiubak` a user can grab with any client, and an
 * interrupted upload simply re-runs. A failure is surfaced per-destination by
 * the caller, so it is visible, not silent.
 *
 * **New uploads never write `.xiaoqiu-parts` again.** Reading it back is kept as
...
class RcloneChunkedUpload(private val context: Context) {
    ...
    fun upload(
        ...
        // mistaken for a backup: the restore list matches `.xiaoqiubak` exactly.
     ...
     * are hidden from the restore list (the filter requires a `.xiaoqiubak`
     ...
     * `.partial` scratch objects, never a user's `.xiaoqiubak` and never the
     * historical `.xiaoqiu-parts` directory.
     */
     ...
     *
     * A `.xiaoqiu-parts/<id>/` upload is a DIRECTORY, and `deletefile` on it
    ...
    data class RemotePackage(
        ...
        val modified: Long?,
        /**
         * >1 only for a `.xiaoqiu-parts` upload written by an older build.
     ...
     *
     * Dotfiles are skipped — that hides `.xiaoqiu-parts` scratch and the
    ...
    /**
     * Everything restorable in [remote]: whole `.xiaoqiubak` packages, plus any
     * chunked upload a PREVIOUS build left behind.
     *
     * In-flight scratch files never show up here: they are named
     * `<package>.xiaoqiubak.partial`, and the filter below requires the name to
     * END in `.xiaoqiubak`.
     */
    fun listPackages(remote: RcloneRemoteStore.Remote): List<RemotePackage> {
        ...
        // BACKWARD COMPATIBILITY ONLY. Nothing writes `.xiaoqiu-parts` any more,
    ...
    fun download(
        ...
        val scratch = File(context.cacheDir, "xiaoqiu-dl-${pkg.displayName}").apply { mkdirs() }
        try {
    ...
    companion object {
         ...
         */
        const val PARTS_DIR = ".xiaoqiu-parts"
