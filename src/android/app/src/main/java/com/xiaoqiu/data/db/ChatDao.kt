 ...
 * the `xiaoqiu-sessions-cli list` offload command). The SELECT shape is
 ...
 * the `xiaoqiu-sessions-cli search` offload command). Same RawQuery
...
interface ChatDao {
    ...
    // ─── T188: xiaoqiu-sessions-cli backing queries ─────────────────────────────
     ...
     * table. Backs `xiaoqiu-sessions-cli search`. SELECT must produce columns
    ...
    /**
     * Paginated message page for `xiaoqiu-sessions-cli messages --offset --limit`.
    ...
    /** Used by `xiaoqiu-sessions-cli messages` to surface the total count
