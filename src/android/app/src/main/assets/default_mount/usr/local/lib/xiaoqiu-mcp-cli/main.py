#!/usr/bin/env python3
"""xiaoqiu-mcp-cli — MCP client CLI for the XiaoQiu agent (iSH / PRoot).
...
Diagnostics go to /var/xiaoqiu/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/xiaoqiu-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/xiaoqiu/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/xiaoqiu-mcp-daemon.pid"
PORT_FILE = "/tmp/xiaoqiu-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/xiaoqiu-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/xiaoqiu/mcp-servers/mcp-daemon.log"
        try:
...
def cmd_add(args, pretty):
    ...
    if wants_oauth:
        out["oauth_note"] = (
            ...
            "the CLI — open XiaoQiu Settings > MCP Integrations > %s and tap "
...
USAGE = """xiaoqiu-mcp-cli — MCP (Model Context Protocol) client for the XiaoQiu agent.

Usage: xiaoqiu-mcp-cli <command> [args] [--pretty]
...
Commands:
  ...
  add ... --oauth-client-id <id> --oauth-auth-endpoint <url> --oauth-token-endpoint <url>
      [--oauth-client-secret <secret>] [--oauth-scopes "s1 s2"] [--oauth-redirect-uri <uri>]
                                        ...
                                        XiaoQiu Settings > MCP Integrations >
                                        <server> > Authorize.
...
Files:
  Servers:  /var/xiaoqiu/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/xiaoqiu/mcp-servers/mcp-cli.log

Examples:
  xiaoqiu-mcp-cli list --pretty
  xiaoqiu-mcp-cli tools notion
  xiaoqiu-mcp-cli call notion search --input '{"q":"x"}'
  xiaoqiu-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  xiaoqiu-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  xiaoqiu-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  xiaoqiu-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
Server startup timeout (XiaoQiu config, not part of the MCP protocol):
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'xiaoqiu-mcp-cli --help' for usage.\n")
