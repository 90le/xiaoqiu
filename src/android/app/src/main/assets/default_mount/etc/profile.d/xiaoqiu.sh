# XiaoQiuApp shell configuration
# Loaded by /etc/profile via the profile.d mechanism (login shells only).

## [小丘] 提示符品牌化：root@xiaoqiu（v2 fork 改名，存量 rootfs 由 RootfsManager 自愈）
# literal "xiaoqiu" into PS1 (deps/prepare_alpine_rootfs.sh) rather than
...
# boot.
export BROWSER=/usr/local/bin/xiaoqiu-open
...
# touched as link sources. Reported as openxiaoqiu/openxiaoqiu#7.
