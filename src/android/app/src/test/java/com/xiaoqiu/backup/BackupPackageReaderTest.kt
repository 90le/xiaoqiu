...
class BackupPackageReaderTest {
    ...
    @Before
    fun setUp() {
        root = File.createTempFile("xiaoqiubak-reader", "").apply { delete(); mkdirs() }
    }
    ...
    fun `refuses a future format major version instead of guessing`() {
        writeManifest("""{"format":"xiaoqiubak/2","backup_id":"future"}""")
    ...
    fun `accepts a same-major minor bump`() {
        writeManifest("""{"format":"xiaoqiubak/1.4","backup_id":"newer-minor"}""")
    ...
    fun `a wrong passphrase fails against the verifier before touching payload`() {
        ...
        writeManifest(
            """
            {"format":"xiaoqiubak/1","encryption":{"scheme":"xiaoqiubak-enc/1",
    ...
    fun `an unsupported encryption scheme is refused`() {
        writeManifest(
            """
            {"format":"xiaoqiubak/1","encryption":{"scheme":"xiaoqiubak-enc/99",
    ...
    fun `an edited manifest fails the sidecar MAC`() {
        ...
        writeManifest(body.replace("xiaoqiubak/1\"", "xiaoqiubak/1\",\"device_name\":\"forged\""))
    ...
    fun `a stripped encryption block is caught when encrypted members remain`() {
        writeManifest("""{"format":"xiaoqiubak/1"}""")
        File(root, "data").mkdirs()
