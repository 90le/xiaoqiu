...
class BackupFileTreeExporterTest {
    ...
    @Before
    fun setUp() {
        tmp = File.createTempFile("xiaoqiubak-tree", "").apply { delete(); mkdirs() }
    ...
    fun `skips backup artifacts so packages never nest`() {
        ...
        File(source, "backup-20260816-1200-abc.xiaoqiubak").writeText("previous package")
        File(source, "Backups").mkdirs()
        File(source, "Backups/backup-older.xiaoqiubak").writeText("older package")
